package com.example.lenovopc.thapareats.Database;
...
public class Database extends SQLiteAssetHelper {
    private static final String DB_NAME = "ThaparEatsDB.db";
    ...
    public void updateCart(Order order) {
        SQLiteDatabase db = getReadableDatabase();
        @SuppressLint("DefaultLocale") String query = String.format("UPDATE OrderDetail SET Quantity = '%s' WHERE UserPhone='%s' AND ProductId='%s';",order.getQuantity(),order.getUserPhone(),order.getProductId());
        db.execSQL(query);
    }

    public void increaseCart(String userPhone,String foodID) {
        SQLiteDatabase db = getReadableDatabase();
        @SuppressLint("DefaultLocale") String query = String.format("UPDATE OrderDetail SET Quantity = Quantity+1 WHERE UserPhone='%s' AND ProductId='%s';",userPhone,foodID);
        db.execSQL(query);
    }
